...
def send_invitation_mail(sender, instance, created, **kwargs):
    ...
    if not created:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                sql = 'CALL update_total_participants(%s)',
                params=[event_id]
            )
        return
    try:
    ...
    try:
        send_invite_mail(f"Invitation for event: {event.title}", context, [instance.receiver_email], html_template='invitation_template.html')
        
        # Schedule reminder mail
        eta_argu = context['event_time'] - timedelta(minutes=2)
        if eta_argu >= timezone.now():
            send_reminder_mail.apply_async((context,[instance.receiver_email]),eta=eta_argu, expires=context['event_time'])
